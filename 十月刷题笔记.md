...
Dijkstra通常是用最小堆解决最短路径问题，求最小的问题就用最小堆来维护，而这一题恰恰相反，求的答案是概率最大，所以这里是用大根堆来维护的，然后就是正常的一些将小于号变成大于号，这样就把这道题解决了



##  2140.解决智力问题(记忆化dfs、dp)
...
public:
    long long mostPoints(vector<vector<int>>& questions) {
        ...
        //状态定义 dfs(i), dp[i]表示解决区间[i,n-1]内的问题可以获得的最高分数
        ...
        auto dfs = [&](auto&& dfs, int i) -> long long {
            ...
            if(res != -1) return res;
            res = 0;
            res = max(dfs(dfs, i+questions[i][1]+1)+questions[i][0], dfs(dfs, i+1));
            return res;
        };
        return dfs(dfs, 0);
    }
...
自己独立写记忆化的时候第一次写错超时了，然后仔细分析了一下状态定义，稍微修改了一下之后还是顺利解决了
...
public:
    int mincostTickets(vector<int>& days, vector<int>& costs) {
        unordered_set<int> ust(days.begin(), days.end());
        int n = days.back();

        vector<int> dp(n+1);
        for(int i = 1; i <= n; i++) {
            if(ust.count(i) == 0) dp[i] = dp[i-1];
            else dp[i] = min({dp[i-1]+costs[0], dp[max(i-7, 0)]+costs[1], dp[max(i-30, 0)]+costs[2]});
        }
        return dp[n];
        ...
        auto dfs = [&](auto&& dfs, int i) -> int {
            ...
            int& res = memo[i];
            if(res != -1) return res;
            if(ust.count(i) == 0) return res = dfs(dfs, i-1);
...
public:
    int longestSquareStreak(vector<int>& nums) {
        ...
        auto dfs = [&](auto&& dfs, int i) -> int {
            int x = sqrt(i);
            if(x*x != i || !ust.count(x)) return 1;
            int& res = memo[i];
            if(res != -1) return res;
            res = 0;
            return res = dfs(dfs, x)+1;
        };
        int ans = 0;
        for(auto& x : ust) {
...
自己写的记忆化搜索超时了，大概也是发现了问题，我想到的是枚举选哪个的方法，但是仔细一想，我每次枚举选哪个都需要枚举整个数组元素，这样枚举确实很慢，超时也就理所应当了。题解是给出的利用哈希集合来O1的查找，确实就快很多了，所以在思考枚举选哪个的时候可以先看看你是枚举的范围在哪，如果每次枚举选哪个的范围是整个数组那么其实和On^2的思路没什么区别了
...
自己的代码又臭又长还写不对，一看题解只有4行。。。原来hash表还能充当dp数组，真是妙到家了



##  2601.质数减法运算(贪心、二分、质数筛预处理)
...
public:
    ...
    bool primeSubOperation(vector<int>& nums) {
        ...
        for(int& x : nums) {
            ...
            int idx = lowerBound(primes, x-pre)-1;
            pre = x-primes[idx];
        }
        return true;
    }
...
这一题很考察不等式的概念，首先需要发现等式关系`p < x-pre`，那么这个p就表示为小于x-pre的最大质数，所以可以用二分来求，也就是`lowerBound(x-pre)-1`，这样就定位到了它的下标。然后就是在二分过程中如果要找小于2的最大质数，按依照以前的写法就会返回下标-1会导致下标越界，所以这里设置了哨兵，也就是在初始化的时候多了一个占位的，这样在使用欧式筛的时候第二层循环j就要从1开始了
...
public:
    vector<int> findRightInterval(vector<vector<int>>& intervals) {
        ...
        vector<int> ans(n, -1);
        for(int i = 0; i < n; i++) {
            auto it = lower_bound(t.begin(), t.end(), make_pair(intervals[i][1], 0));
            if(it != t.end()) ans[i] = it->second;
        }
        return ans;
    }
};
...
用t来维护一个左边界和下标，然后遍历区间，二分查找是否存在大于等于`intervals[i][1]`的左边界，要求最小化，所以就是用lower_bound。这一题不熟悉就是在二分查找的时候是查找的一个pair，应该准确来说是pair的first，得保证是相同的容器，代表必须大于等于这一整个pair，以0为开始是因为数组下标从0开始
