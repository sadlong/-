...
在C++中有专门的api得到一个数组的全排列，只不过为了正确得到一个数组的全排列，应当先`sort`一下数组再调用`next_permutation`
...
public:
    int countSpecialNumbers(int n) {
        ...
        int m = s.size();
        //mask表示前面选过数字的集合    这个参数不是必须 是为了解决这个题而设置的mask
        ...
        auto dfs = [&](auto&& dfs, int i, int mask, bool isLimit, bool isNum) -> int {
            ...
            int res = 0;
            //可以跳过当前数位
            if(!isNum) res += dfs(dfs, i+1, mask, false, false);
            //确定上界
            int up = isLimit ? s[i]-'0' : 9;
            //枚举数字
            for(int d = isNum ? 0 : 1; d <= up; d++) {
                //没选过
                if((mask>>d&1) == 0) res += dfs(dfs, i+1, mask|(1<<d), isLimit && d == up, true);
            }
...
关键的关键在于理解为什么`isLimit`和`isNum`为什么不需要记忆化？

因为当`isLimit`为true的时候只有一种情况，就是和n相等，在递归的过程中只会遇到一次。同理当`isNum`为false的时候表示前面的数位都跳过了，这也是只会出现一次的情况，所以直接跳过。总的来说，其实前面两个参数就已经能够确定`isLimit`和`isNum`的情况了，所以这两个参数并不需要记忆化
...
通常要往以i下标为结尾的符合条件的子数组长度，最后的答案一般是`ans += r-l+1`或者`ans += f[i]`之类的结果，第一种情况比较符合滑动窗口，第二种就比较符合前缀和或者动态规划
