    ...
    for(int i = 0; i < g.size(); i++) {
        for(int j = i+1; j < g.size(); j++) {
            ...
            for(int k = 0; k < g.size(); k++) {
                auto a = g[i], b = g[j], c = g[k];
                auto check = [&]() -> bool {
                    if(abs(a.first-c.first)+abs(a.second-c.second) <= d || 
                    abs(b.first-c.first)+abs(b.second-c.second) <= d) return true;
                    return false;
                };
                cnt += check();
            }
...
void solve() {
    ...
    for(int i = 1; i <= 2*n; i++) pre[i] = pre[i-1]+a[i-1];
